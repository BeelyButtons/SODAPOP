...
export const applicationSchema = z.object({
  ...
  wineAppellation: z.string().trim().optional(),
  wineAppellationType: z.enum(['country', 'state', 'county', 'ava', 'foreign_country', 'foreign_subdivision', 'foreign_viticultural_area']).optional(),
  wineAppellationPercentage: z.number().min(0).max(100).optional(),
  wineFinishedInRequiredArea: z.boolean().optional(),
  wineVarietals: z.array(z.object({
    name: z.string().trim().min(1),
    percentage: z.number().min(0).max(100),
  })).optional(),
  wineVintage: z.string().trim().optional(),
  wineVintagePercentage: z.number().min(0).max(100).optional(),
  wineEstateBottledClaim: z.boolean().optional(),
  wineEstateProductionContinuous: z.boolean().optional(),
