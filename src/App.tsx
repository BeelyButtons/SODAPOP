...
function Welcome({ onContinue }: { onContinue: () => void }) {
  return <main className="welcome-page">
    ...
    <section className="welcome-hero"><p className="eyebrow">LabelEvidence prototype</p><h1>Evidence-led alcohol label review, built around the human decision.</h1><p>LabelEvidence compares product application information with the submitted label image and the requirements that apply to that product.</p><button className="primary-button" type="button" onClick={onContinue}>Go to LabelEvidence</button></section>
    <section className="welcome-guide" aria-labelledby="welcome-guide-title"><h2 id="welcome-guide-title">How to use LabelEvidence</h2><p className="welcome-guide-intro">To begin the label review process, click <strong>Go to LabelEvidence</strong>. You will enter the active review queue, where individual applications and batches wait for AI analysis and human decisions.</p><ol>
      <li><h3>Start the analysis</h3><p>Select <strong>Begin AI analysis</strong>. LabelEvidence reads the applications in queue order, selects the applicable requirements, examines each label image with OCR and image analysis, and compares the image evidence with the application.</p></li>
      <li><h3>Begin human review</h3><p>Select <strong>Review label</strong> for an evaluated application, or <strong>Begin or resume human review</strong> to move through the queue in order. Human review can start while later labels are still being analyzed.</p></li>
      <li><h3>Inspect the image</h3><p>Open <strong>Edit / enhance image</strong> to zoom, rotate, or adjust brightness and contrast. These viewing tools help with difficult text without changing the original submitted evidence.</p></li>
      <li><h3>Resolve AI concerns</h3><p>Select <strong>Confirm concern</strong> when the AI found a genuine issue, or <strong>Dismiss concern</strong> when the concern is a false alarm or does not require correction.</p></li>
      <li><h3>Challenge a cleared check</h3><p>When the AI detects no concerns, open <strong>View complete review</strong>. Select <strong>Disagree</strong> beside any item the AI cleared but the reviewer believes is problematic.</p></li>
      <li><h3>Make the decision</h3><p>Select <strong>Approve label</strong> or <strong>Return for correction</strong>, add a note when needed, and then select <strong>Confirm and proceed to next label</strong>.</p></li>
      <li><h3>Work with batches and history</h3><p>Review concerning batch labels individually. Clear batch labels may be approved together after reviewer attestation. Confirmed decisions move to <strong>Completed reviews</strong>, where they can be reopened and changed.</p></li>
    </ol></section>
    <section className="welcome-more"><article><p className="eyebrow">What happens in the background</p><h2>Application-guided image review</h2><p>The application determines which requirements apply. Each requirement becomes an evidence question for the image reader. Missing, uncertain, or conflicting evidence becomes a specific human-review concern. Image uncertainty is kept separate from a readable compliance conflict.</p></article><article><p className="eyebrow">Reduce avoidable submissions</p><h2>Applicant prescreen</h2><p>The applicant-facing demonstration uses editable Domestic Distilled Spirits application information and an uploaded label image to identify possible omissions or conflicts before official submission. Its results are advisory and do not guarantee approval.</p></article></section>
